...
namespace Ship.Game
{
    public class TestBox
    {
        ...
        public TestBox(Texture2D mainTextures, TextureAtlas mainAtlas, GraphicsDevice graphicsDevice, Texture2D grassTexture)
        {
            _grassTexture = grassTexture;
            _mainAtlas = mainAtlas;
            _mainTextures = mainTextures;
            _rect = new Rectangle(0, 0, MainGame.SectorTileSize, MainGame.SectorTileSize);
        ...
        public void TileTesting(GraphicsDevice graphicsDevice)
        {
            ...
            var xspot = 1;
            var yspot = 1;
            showPosition = new Vector2(((xspot*MainGame.SectorTileSize)*Zoom) - (textureRegion.Bounds.Width/2.0f),
                                       ((yspot*MainGame.SectorTileSize)*Zoom) - (textureRegion.Bounds.Height*3) +
                                       ((MainGame.SectorTileSize*Zoom)/2));
            //TerrainTesting(graphicsDevice); 
            ...
            for (var x = 0; x < _texture.GetLength(0); x++)
            {
                ...
                {
                    _texture[x, y] = new Texture2D(graphicsDevice, MainGame.SectorTileSize, MainGame.SectorTileSize);
                    _texturePos[x, y] = new Vector2((x*MainGame.SectorTileSize)*Zoom, (y*MainGame.SectorTileSize)*Zoom);


                    for (var i = 0; i < MainGame.SectorTileSize; i++)
                    {
                        for (var j = 0; j < MainGame.SectorTileSize; j++)
                        {
                            if (i == 0 || j == 0)
        ...
        public void TerrainTesting(GraphicsDevice graphicsDevice)
        {
            var red = new[] {Color.Red};
            var black = new[] {Color.Black};
            ...
            System.Diagnostics.Debug.WriteLine("rand:{0}", nextRand);
            ...
            while(true)
            {
                ...
                for (var x = 0; x < _texture.GetLength(0); x++)
                {
                    for (var y = 0; y < _texture.GetLength(0); y++)
                    {
                        if (MainGame.WorldData.WaterSquareData[x + xCord][y + yCord] != null)
                            breakout = true;
            ...
            for (var x = 0; x < _texture.GetLength(0); x++)
            {
                for (var y = 0; y < _texture.GetLength(0); y++)
                {
                    ...
                    var tiledata = GetTestWater(MainGame.WorldData.WaterSquareData, x + xCord, y + yCord, newRand);
                    //if (oceanSquares[x + 1][y + 1] != waterVal) continue;
                    //var tiledata = NoiseGen.OceanWithLand(MainGame.SectorTileSize, oceanSquares, x + 1, y + 1, waterVal);
        ...
        public byte[,] GetTestWater(WaterSquare[][] masterWaterData, int x, int y,Random rand)
        {
            var s = Environment.TickCount;
            var data = new byte[MainGame.SectorTileSize,MainGame.SectorTileSize];
        ...
        {
            var td = new byte[MainGame.SectorTileSize,MainGame.SectorTileSize];
