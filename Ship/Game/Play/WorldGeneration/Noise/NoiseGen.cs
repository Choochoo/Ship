...
namespace Ship.Game.Play.WorldGeneration.Noise
{
    public class NoiseGen
    {
        ...
        private static byte[,] _newRiver = new byte[PlayScreen.SectorTileSize,PlayScreen.SectorTileSize];
        private static byte[,] _newerRiver = new byte[PlayScreen.SectorTileSize,PlayScreen.SectorTileSize];
        ...
        {
            const byte size = PlayScreen.SectorTileSize;
            for (var x = 0; x < size; x++)
            {
                for (var y = 0; y < size; y++)
                {
        ...
        private static void CreateRiverBezier(ref byte[,] output, byte thickness, byte val, bool topEqual,
                                              bool bottomEqual,
                                              bool leftEqual, bool rightEqual, bool createLake, Random rand)
        {
            ...
            const int half = PlayScreen.SectorTileSize/2;
            ...
            if (bottomEqual)
            {
                ...
                {
                    startx = half;
                    starty = PlayScreen.SectorTileSize;
                }
                else
                {
                    endx = half;
                    endy = PlayScreen.SectorTileSize;
                }
            }
            if (leftEqual)
            {
            ...
            if (rightEqual)
            {
                endx = PlayScreen.SectorTileSize;
                endy = half;
            }
            ...
            for (var i = 1; i < PointsOnRiver - 1; i += 2)
            {
                ...
                outx = outx > PlayScreen.SectorTileSize - 1 ? PlayScreen.SectorTileSize - 1 : outx;
                outy = outy > PlayScreen.SectorTileSize - 1 ? PlayScreen.SectorTileSize - 1 : outy;
...
//lastpointy = 33, outy = 32
            ...
            output[
                startx == PlayScreen.SectorTileSize ? PlayScreen.SectorTileSize - 1 : startx,
                starty == PlayScreen.SectorTileSize ? PlayScreen.SectorTileSize - 1 : starty] = val;
            output[
                endx == PlayScreen.SectorTileSize ? PlayScreen.SectorTileSize - 1 : endx,
                endy == PlayScreen.SectorTileSize ? PlayScreen.SectorTileSize - 1 : endy] = val;
            ...
            for (var y = 0; y < PlayScreen.SectorTileSize; y++)
            {
                for (var x = 0; x < PlayScreen.SectorTileSize; x++)
                {
                    ...
                    var outx3 = x + rightthick >= PlayScreen.SectorTileSize
                                    ? PlayScreen.SectorTileSize - 1
                                    : x + rightthick;
                    var outy3 = y + rightthick >= PlayScreen.SectorTileSize
                                    ? PlayScreen.SectorTileSize - 1
                                    : y + rightthick;
        ...
        private static void CreateOceanBezier(ref byte[,] output, int[] spots, byte landval, byte fillDirection,
                                              Random rand,
        ...
        private static void GetLake(ref byte[,] output, byte thickness, Random rand)
        {
            ...
            //            xPos = xPos > PlayScreen.SectorTileSize - 1 ? PlayScreen.SectorTileSize - 1 : xPos;
            //            yPos = yPos > PlayScreen.SectorTileSize - 1 ? PlayScreen.SectorTileSize - 1 : yPos;
            //            output[xPos, yPos] = 1;
            //        }
            //    }
            //}
        ...
        public static void OceanWithLand(ref byte[,] output, byte[][] waterData, int x, int y, byte val)
        {
            ...
            {
                CreateOceanBezier(ref output, new[] {oneThird, 0, oneThird, half, oneThird, size}, val, FillLeftToRight,
                                  rand);
                //topLeftNum = 12;
                //topRightNum = 13;
                //bottomLeftNum = 16;
            ...
            else if (midLeftFine && !midRightFine && topMidFine && bottomMidFine)
            {
                ...
                //bottomLeftNum = 17;
                //bottomRightNum = 19;
                CreateOceanBezier(ref output, new[] {twoThird, 0, twoThird, half, twoThird, size}, val, FillRightToLeft,
                                  rand);
                ...
                if (!topLeftFine)
                {
                    CreateOceanBezier(ref output, new[] {oneThird, 0, oneThird, oneThird, 0, oneThird}, val, FillTopLeft,
                                      rand,
                                      false, true);
                }
            }
            else if (midLeftFine && midRightFine && !topMidFine && bottomMidFine)
            {
                ...
                //bottomLeftNum = 13;
                //bottomRightNum = 14;
                CreateOceanBezier(ref output, new[] {0, oneThird, half, oneThird, size, oneThird}, val, FillTopToBottom,
                                  rand,
                                  true);
                if (!bottomLeftFine)
                {
            ...
            {
                CreateOceanBezier(ref output, new[] {0, twoThird, half, twoThird, size, twoThird}, val, FillBottomToTop,
                                  rand);
                //topLeftNum = 13;
                //topRightNum = 14;
                //bottomLeftNum = 21;
            ...
            {
                CreateOceanBezier(ref output, new[] {twoThird, 0, twoThird, twoThird, 0, twoThird}, val, FillBottomRight,
                                  rand);
                //topLeftNum = 13;
                //topRightNum = 15;
                //bottomLeftNum = 21;
        ...
        public static void AddSand(ref byte[,] output, byte waterVal, byte grassVal, byte sandVal, int sizeMin,
                                   int sizeMax, Random rand)
        {
            var size = output.GetLength(0);
